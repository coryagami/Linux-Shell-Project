...
void execCmd(arglist* arg_list, bool is_background_process, bool is_ioacct_cmd)
{
	...
	if (pipes == 1)
	{
			forkAChild(&arg_list[1], pipes, 1, is_background_process, is_ioacct_cmd, is_in_redir, is_out_redir,
	    	                   is_append_redir, redir_file, cmd_path[1], pipefd);
			forkAChild(&arg_list[0], pipes, 2, is_background_process, is_ioacct_cmd, is_in_redir, is_out_redir,
	    	                   is_append_redir, redir_file, cmd_path[0], pipefd);
	}
	if (pipes == 2)
	{
